...
function M.setup()
	...
	local function map(mode, key, cmd, options, description)
		local opts = vim.tbl_deep_extend("force", { noremap = true, silent = true, desc = description }, options or {})
		vim.keymap.set(mode, key, cmd, opts)
	end

	-- Helper function for buffer-local keymap definitions
	local function buf_map(bufnr, mode, key, cmd, description)
		vim.api.nvim_buf_set_keymap(bufnr or 0, mode, key, cmd, { noremap = true, silent = true, desc = description })
	end
	...
	function _G.set_markdown_keymaps()
		...
		if ok and autolist and autolist.operations then
			...
			vim.keymap.set(
				"i",
				"<CR>",
				autolist.operations.enter_handler,
			...
			vim.keymap.set(
				"i",
				"<Tab>",
				autolist.operations.tab_handler,
			...
			vim.keymap.set(
				"i",
				"<S-Tab>",
				autolist.operations.shift_tab_handler,
			...
			vim.keymap.set(
				"n",
				"<C-n>",
				"<cmd>AutolistIncrementCheckbox<CR>",
			...
			vim.keymap.set(
				"n",
				"<A-n>",
				"<cmd>AutolistDecrementCheckbox<CR>",
				{ buffer = true, desc = "Decrement checkbox" }
			)

			vim.keymap.set("n", "o", "o<cmd>AutolistNewBullet<cr>", { buffer = true, desc = "New bullet below" })

			vim.keymap.set("n", "O", "O<cmd>AutolistNewBulletBefore<cr>", { buffer = true, desc = "New bullet above" })

			vim.keymap.set("n", ">", "><cmd>AutolistRecalculate<cr>", { buffer = true, desc = "Indent bullet" })

			vim.keymap.set("n", "<", "<<cmd>AutolistRecalculate<cr>", { buffer = true, desc = "Unindent bullet" })
	...
	map("n", "<C-n>", "<cmd>NvimTreeToggle<CR>") -- toggle file explorer
		...
		if _G.GotoBuffer then
			...
			local function safe_buffer_next()
				...
				for _, buf in ipairs(buffers) do
					...
					if
						vim.api.nvim_buf_is_valid(buf.bufnr)
						and vim.api.nvim_buf_get_option(buf.bufnr, "buftype") == ""
					then
	...
	map("n", "<C-j>", "gj", {}, "move down display line")
	map("n", "<C-k>", "gk", {}, "move up display line")
	map("v", "<C-j>", "gj", {}, "move down display line")
	map("v", "<C-k>", "gk", {}, "move up display line")
