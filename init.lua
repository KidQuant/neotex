...
vim.notify = function(msg, level, opts)
	...
	if
		...
		and type(msg) == "string"
		and (
			msg:match("Client stylua quit with exit code")
			or msg:match("stylua.*stderr")
			or msg:match("Could not start dynamically linked executable: stylua")
		)
	then
		return
	end
	...
	if msg and type(msg) == "string" and (msg:match("framework.*deprecated") or msg:match("vim.lsp.config")) then
		return
	end
	...
	if
		...
		and type(msg) == "string"
		and (
			msg:match("Failed to load plugin module:.*firenvim")
			or msg:match("Failed to load plugin module:.*wezterm%-integration")
		)
	then
		return
	end
...
if not config_ok then
	vim.notify(
		"Error loading config structure: " .. tostring(config) .. ". Using minimal fallback.",
		vim.log.levels.WARN
	)
